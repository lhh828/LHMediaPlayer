 ...
 * 控制器抽象类
 */
public abstract class HaoVideoPlayerController
        extends FrameLayout implements View.OnTouchListener {

    private Context mContext;
    protected IHaoVideoPlayer mHaoVideoPlayer;
    ...
    public HaoVideoPlayerController(Context context) {
        super(context);
        mContext = context;
        this.setOnTouchListener(this);
    }

    public void setHaoVideoPlayer(IHaoVideoPlayer haoVideoPlayer) {
        mHaoVideoPlayer = haoVideoPlayer;
    }
     ...
     *                  <ul>
     *                  <li>{@link HaoVideoPlayer#STATE_IDLE}</li>
     *                  <li>{@link HaoVideoPlayer#STATE_PREPARING}</li>
     *                  <li>{@link HaoVideoPlayer#STATE_PREPARED}</li>
     *                  <li>{@link HaoVideoPlayer#STATE_PLAYING}</li>
     *                  <li>{@link HaoVideoPlayer#STATE_PAUSED}</li>
     *                  <li>{@link HaoVideoPlayer#STATE_BUFFERING_PLAYING}</li>
     *                  <li>{@link HaoVideoPlayer#STATE_BUFFERING_PAUSED}</li>
     *                  <li>{@link HaoVideoPlayer#STATE_ERROR}</li>
     *                  <li>{@link HaoVideoPlayer#STATE_COMPLETED}</li>
     *                  </ul>
     */
     ...
     *                 <ul>
     *                 <li>{@link HaoVideoPlayer#MODE_NORMAL}</li>
     *                 <li>{@link HaoVideoPlayer#MODE_FULL_SCREEN}</li>
     *                 <li>{@link HaoVideoPlayer#MODE_TINY_WINDOW}</li>
     *                 </ul>
     */
    ...
    protected void startUpdateProgressTimer() {
        ...
        if (mUpdateProgressTimerTask == null) {
            mUpdateProgressTimerTask = new TimerTask() {
                @Override
                public void run() {
                    HaoVideoPlayerController.this.post(new Runnable() {
                        @Override
                        public void run() {
                            updateProgress();
                        }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (mHaoVideoPlayer.isIdle()
                || mHaoVideoPlayer.isError()
                || mHaoVideoPlayer.isPreparing()
                || mHaoVideoPlayer.isPrepared()
                || mHaoVideoPlayer.isCompleted()) {
            hideChangePosition();
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (!mNeedChangePosition && !mNeedChangeVolume && !mNeedChangeBrightness) {
                    ...
                    if (absDeltaX >= THRESHOLD) {
                        ...
                        mGestureDownPosition = mHaoVideoPlayer.getCurrentPosition();
                    } else if (absDeltaY >= THRESHOLD) {
                        if (mDownX < getWidth() * 0.5f) {
                            // 左侧改变亮度
                            mNeedChangeBrightness = true;
                            mGestureDownBrightness = HaoUtil.scanForActivity(mContext)
                                    .getWindow().getAttributes().screenBrightness;
                        } else {
                            // 右侧改变声音
                            mNeedChangeVolume = true;
                            mGestureDownVolume = mHaoVideoPlayer.getVolume();
                        }
                    }
                }
            ...
            case MotionEvent.ACTION_UP:
                if (mNeedChangePosition) {
                    mHaoVideoPlayer.seekTo((int)mNewPosition);
                    hideChangePosition();
